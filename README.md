...
```shell
# Clean up exited Docker containers first (if any)
...
To test AI-assisted programming, we add a method `UpdateBody` to the article entity in the model, which is used to update the body of the article.
...
Start an aos process in a terminal:

```shell
aos ai_ao_test
```

This will start an AO process and enter the aos REPL (Read-Eval-Print Loop) environment.
...
### Test Creating/Updating/Reading Articles
...
Now, check the "article sequence Id" again:
...
### More Complex Example of AO Dapp Low-Code development
